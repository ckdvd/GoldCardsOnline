package com.tengman.db26.utils.product;
...
import com.tengman.db26.utils.MathUtils.MathUtils;
...
//		Mat structImage = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(level, level));

		/**
		 * 图像腐蚀 腐蚀说明： 图像的一部分区域与指定的核进行卷积， 求核的最`小`值并赋值给指定区域。 腐蚀可以理解为图像中`高亮区域`的'领域缩小'。
		 * 意思是高亮部分会被不是高亮部分的像素侵蚀掉，使高亮部分越来越少。
		 */
		Imgproc.erode(src, outImage, new Mat());
//		Imgproc.erode(src, outImage, structImage);
		src = outImage;
		/**
		 * 膨胀 膨胀说明： 图像的一部分区域与指定的核进行卷积， 求核的最`大`值并赋值给指定区域。 膨胀可以理解为图像中`高亮区域`的'领域扩大'。
		 * 意思是高亮部分会侵蚀不是高亮的部分，使高亮部分越来越多。
		 */
//		Imgproc.dilate(src, outImage, structImage, new Point(-1, -1), level);
		Imgproc.dilate(src, outImage, new Mat());
		src = outImage;
