package com.tengman.db26.utils.old.rewrite;
...
public class ImgUtils {
    ...
    private static final int WHITE = 245;
    ...
    public static List<Mat> _cutImgX(Mat src) {
        return _cutImgX(src, 20);
    }

    public static List<Mat> _cutImgX(Mat src, int perHeight) {
        int i, j;
        int width = getImgWidth(src), height = getImgHeight(src);
        int[] xNum, cNum;
        int average = 0;// 记录黑色像素和的平均值
        ...
        int zipLine = 3;
        //压缩像素值数量；即统计三行像素值的数量为一行// 统计出每行白色像素点的个数
        xNum = zipLinePixel(countPixel(src, height, width, true), zipLine);

        //排序
        cNum = Arrays.copyOf(xNum, xNum.length);
        Arrays.sort(cNum);//从小到大排序
        for (i = 0; i < cNum.length / 32; i++) {//1/32的值，也就是最大的，最黑的
            average += cNum[i];
        }
...
//			System.out.println(h);
     ...
     * @return
     */
    public static List<Mat> _cutImgY(Mat src, int margin) {
        int i, j;
        int width = getImgWidth(src), height = getImgHeight(src);
        int[] yNum, cNum;
        int average = 0;// 记录黑色像素和的平均值
        ...
        // 经过测试这样得到的平均值最优，平均值的选取很重要
        cNum = Arrays.copyOf(yNum, yNum.length);//数组拷贝
        Arrays.sort(cNum);//从小到大排序
        for (i = 0; i < cNum.length / 32; i++) {//1/32的值，也就是最大的，最黑的
            average += cNum[i];
        }
        average /= (cNum.length / 32);//求平均值
        ...
        // 优化cutX
        if (cutX.size() != 0) {
            int temp = cutX.get(cutX.size() - 1);//最后一根
            // 因为线条有粗细，优化cutX
            for (i = cutX.size() - 2; i >= 0; i--) {//倒数第二跟开始
                int k = temp - cutX.get(i);
                if (k <= margin) {
                    cutX.remove(i);
                } else {
...
//			temp = cutX.get(cutX.size() - 1);//最后一根
//			// 因为线条有粗细，优化cutX
//			for (i = cutX.size() - 2; i >= 0; i--) {//倒数第二跟开始
//				int k = temp - cutX.get(i);
//				if (k <= margin) {
...
//		}
     ...
     * 优化参考点集合
     *
     * @param referencePoints 参照点
     * @param cannyMat        图像Mat矩阵
     * @return
     */
