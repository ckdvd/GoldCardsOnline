package com.tengman.db26.utils.old.utils;
...
public class HandleImgUtils {
    ...
    public static Mat canny(Mat src) {
        Mat mat = src.clone();
        Imgproc.Canny(src, mat, 60, 150);
//		HandleImgUtils.saveImg(mat, "C:/Users/admin/Desktop/opencv/open/x/canny.jpg");
        return mat;
    }

    public static Mat canny(Mat src, int threshold1, int shreshold2) {
        Mat mat = src.clone();
        Imgproc.Canny(src, mat, threshold1, shreshold2);
//		HandleImgUtils.saveImg(mat, "C:/Users/admin/Desktop/opencv/open/x/canny.jpg");
        return mat;
    }
    ...
    public static Mat laplacian(Mat src) {
        Mat mat = src.clone();
        Imgproc.Laplacian(src, mat, CvType.CV_8UC1);
//		HandleImgUtils.saveImg(mat, "C:/Users/admin/Desktop/opencv/open/x/canny.jpg");
        return mat;
    }
    ...
    public static Mat scharr(Mat src) {
        Mat mat = src.clone();
        Imgproc.Scharr(src, mat, CvType.CV_64FC1, 0, 0);
//		HandleImgUtils.saveImg(mat, "C:/Users/admin/Desktop/opencv/open/x/canny.jpg");
        return mat;
    }
    ...
    public static Rect findMaxRect2(Mat cannyMat) {
        List<MatOfPoint> contours = new ArrayList<MatOfPoint>();
        Mat hierarchy = new Mat();
...
//        Point[] points = HandleImgUtils.findFourPoint(src);
        ...
        Mat cannyMat = HandleImgUtils.canny(src);
        // 寻找最大矩形
        RotatedRect rotatedRect = HandleImgUtils.findMaxRect(cannyMat);
        // 点的顺序[左上 ，右上 ，右下 ，左下]
        // 获取旋转矩阵和新图片尺寸
        ...
        do {
            size.width += 1;
            size.height += 1;
        } while ((int) size.width % 2 != 0 || (int) size.height % 2 != 0); // 确保尺寸为偶数

        Mat rotMat = Imgproc.getRotationMatrix2D(center, -rotatedRect.angle, 1.0);
        Mat warpMat = new Mat(2, 3, CvType.CV_32F);
        rotMat.convertTo(warpMat, CvType.CV_32F);

        // 矫正图片
        Mat corrected = new Mat();
        Imgproc.warpAffine(src, corrected, warpMat, new Size((int) size.width, (int) size.height), Imgproc.INTER_LINEAR + Imgproc.WARP_FILL_OUTLIERS);
    ...
    public static Mat cutRect(RotatedRect rect, Mat nativeCorrectMat) {
        return cutRect(rect, nativeCorrectMat, 2, 10);
    }

    public static Mat cutRect(RotatedRect rect, Mat nativeCorrectMat, int offsetX, int offsetY) {
        //有时候框歪了，设置偏移量，人为干预，前提是卡槽在中间才能这么做
        Point center = rect.center;
        int xMove = (int) (nativeCorrectMat.width() / 2 - center.x);
        int yMove = (int) (nativeCorrectMat.height() / 2 - center.y);
        //校正或者放弃
//        if (offsetX > 0) {
//            double[] moveParam = {Math.abs(xMove) > offsetX ? center.x + xMove / 2 : center.x, center.y, rect.size.width, rect.size.height};
//            rect.set(moveParam);
//        }
//        if (offsetX > 0 && offsetY > 0) {
//            double[] moveParam = {Math.abs(xMove) > offsetX ? center.x + xMove/2 : center.x, Math.abs(yMove) > offsetY ? center.y + yMove / 2 : center.y, rect.size.width, rect.size.height};
//            rect.set(moveParam);
//        }
        //设置边界
        Rect roi1 = rect.boundingRect();
        setBounds(nativeCorrectMat, roi1);
        return new Mat(nativeCorrectMat, roi1);
    }
    ...
    public static Mat cutRect(Mat correctMat, Mat nativeCorrectMat) {
        ...
        return cutRect(rect, nativeCorrectMat);
//		HandleImgUtils.saveImg(t, "C:/Users/admin/Desktop/opencv/open/x/cutRect.jpg");
    }

    public static void setBounds(Mat nativeCorrectMat, Rect roi1) {
        if (roi1.x < 0) {
            roi1.x = 0;
        }
        if (roi1.y < 0) {
        ...
        if (roi1.width + roi1.x > nativeCorrectMat.width()) {
            roi1.width = nativeCorrectMat.width() - roi1.x;
        }
        if (roi1.height + roi1.y > nativeCorrectMat.height()) {
            roi1.height = nativeCorrectMat.height() - roi1.y;
        }
    }
     ...
     * @param correctMat 图像矫正后的Mat矩阵
     */
    public static Mat cutRect2(Mat correctMat, Mat nativeCorrectMat) {
        Mat t = new Mat();
        // 获取最大矩形
//        RotatedRect rect = findMaxRect(correctMat);
        Rect rect1 = findMaxRect2(correctMat);
        if (rect1 == null) {
            return t;
        }
//        Rect rect1 = rect.boundingRect();
        Mat temp = new Mat(nativeCorrectMat, rect1);
        temp.copyTo(t);
        return t;
//		HandleImgUtils.saveImg(t, "C:/Users/admin/Desktop/opencv/open/x/cutRect.jpg");
    }
